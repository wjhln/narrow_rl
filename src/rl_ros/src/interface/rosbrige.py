...
def _top_laser_scan_callback(msg):
    ...
    poly_x = [0.61, 0.57, -0.158, -0.258, -0.258, -0.158, 0.57, 0.61]
    poly_y = [-0.29, -0.38, -0.38, -0.278, 0.278, 0.38, 0.38, 0.29]
...
def _laser_scan_callback(msg):
    ...
    id = 0
    for i, (x, y) in enumerate(points[laser_midindex - 30 -1:laser_midindex + 30]):
        marker = Marker()
        marker.header.frame_id = "base_link"  # 设置参考坐标系
        marker.header.stamp = rospy.Time.now()
        marker.ns = "points"
        marker.id = id  # 每个点的唯一标识符
    ...
    for i, (x, y) in enumerate(points[laser_midindex - 95 -1:laser_midindex - 85]):
        marker = Marker()
        marker.header.frame_id = "base_link"  # 设置参考坐标系
        marker.header.stamp = rospy.Time.now()
        marker.ns = "points"
    ...
    for i, (x, y) in enumerate(points[laser_midindex + 85 -1:laser_midindex + 95]):
        marker = Marker()
        marker.header.frame_id = "base_link"  # 设置参考坐标系
        marker.header.stamp = rospy.Time.now()
        marker.ns = "points"
